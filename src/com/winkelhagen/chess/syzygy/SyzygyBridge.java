...
public class SyzygyBridge {
    ...
    static {
        try {
            ...
            Chess.getLogger().printFormat(LoggerLevel.INFO, "looking for %s at location %s", libName, libFile);
            if (libFile.exists()) {
                System.load(libFile.getAbsolutePath());
                Chess.getLogger().printFormat(LoggerLevel.INFO, "loaded %s located next to the .jar file", libName);
            } else {
                URL classpathLibUrl = SyzygyBridge.class.getClassLoader().getResource(libName);
                Chess.getLogger().printFormat(LoggerLevel.INFO, "looking for %s at location %s", libName, classpathLibUrl);
                if (classpathLibUrl != null && FILE_SCHEME.equalsIgnoreCase(classpathLibUrl.toURI().getScheme()) && Paths.get(classpathLibUrl.toURI()).toFile().exists()){
                    ...
                    Chess.getLogger().printFormat(LoggerLevel.INFO, "loaded %s located in the resources directory", libName);
                } else {
                    Chess.getLogger().printFormat(LoggerLevel.INFO, "looking for %s at java.library.path: %s", libName, System.getProperty("java.library.path"));
                    System.loadLibrary("JSyzygy");
                    Chess.getLogger().printFormat(LoggerLevel.INFO, "loaded %s located in the java library path", libName);
                }
            }
            libLoaded = true;
        } catch (URISyntaxException | UnsatisfiedLinkError | RuntimeException e) {
            Chess.getLogger().printFormat(LoggerLevel.WARNING,  "unable to load JSyzygy library", e);
        }
    }
    ...
    public static synchronized int load(String path){
        Chess.getLogger().printFormat(LoggerLevel.INFO, "loading syzygy tablebases from %s", path);
        if (tbLargest>0){
            Chess.getLogger().printFormat(LoggerLevel.WARNING, "Syzygy tablebases are already loaded");
            return tbLargest;
        }
